...
/**
 * mmc_gpiod_request_reset - request a gpio descriptor for device reset
 * @host: mmc host
 * @con_id: function within the GPIO consumer
 * @idx: index of the GPIO to obtain in the consumer
 * @debounce: debounce time in microseconds
 *
 * Use this function in place of mmc_gpio_request_rs() to use the GPIO
 * descriptor API.  Note that it is paired with mmc_gpiod_free_reset()
 * (which doesn't exist), not mmc_gpio_free_rs().
 *
 * Returns zero on success, else an error.
 */
int mmc_gpiod_request_reset(struct mmc_host *host, const char *con_id,
			    unsigned int idx,
			    unsigned int debounce)
{
	struct mmc_gpio *ctx;
	...
	desc = devm_gpiod_get_index(host->parent, con_id, idx, GPIOD_OUT_LOW);
	if (IS_ERR(desc))
		return PTR_ERR(desc);
...
EXPORT_SYMBOL(mmc_gpiod_request_reset);
