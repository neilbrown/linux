			   ...
			   struct osc_async_page *oap, int bytes)
{
	 ...
	 * get evicted if we wait for a normal obd_timeout or some such.  So
	 * we try to wait half the time it would take the client to be evicted
	 * by server which is half obd_timeout when AT is off or at least
	 * ldlm_enqueue_min with AT on.  See LU-13131
	 */
	unsigned long timeout =
		HZ * (AT_OFF ? obd_timeout : ldlm_enqueue_min) / 2;
			 ...
			 int brw_flags)
{
	...
	if (!ext->oe_rw) { /* write */
		if (!ext->oe_srvlock && !ext->oe_dio) {
			/*
			 * The most likely case here is from lack of grants so
			 * we are either out of quota or out of space.  Since
			 * this means we are holding locks across potentially
			 * multi-striped IO, we must send out everything out
			 * instantly to avoid prolonged waits resulting in
			 * lock eviction (likely since the extended wait in
			 * osc_cache_enter() did not yield any additional
			 * grant due to a timeout.  LU-13131
			 */
			ext->oe_hp = 1;
