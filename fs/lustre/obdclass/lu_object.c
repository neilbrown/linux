...
int lu_site_init(struct lu_site *s, struct lu_device *top)
{
	...
	s->ls_bkts = kvcalloc(s->ls_bkt_cnt, sizeof(*s->ls_bkts), GFP_KERNEL);
	if (!s->ls_bkts) {
...
{
	ctx->lc_value = kcalloc(ARRAY_SIZE(lu_keys), sizeof(*ctx->lc_value),
				GFP_NOFS);
...
void lu_buf_alloc(struct lu_buf *buf, size_t size)
{
	...
	LASSERT(buf->lb_len == 0);
	buf->lb_buf = kvzalloc(size, GFP_KERNEL);
