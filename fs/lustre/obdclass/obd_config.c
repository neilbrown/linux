				     ...
				     const char *new_name)
{
	...
	new_cfg = kzalloc(lustre_cfg_len(bufs->lcfg_bufcount, bufs->lcfg_buflen),
			  GFP_NOFS);
	if (!new_cfg) {
		new_cfg = ERR_PTR(-ENOMEM);
		goto out_free_buf;
			      ...
			      struct llog_rec_hdr *rec, void *data)
{
	...
	case OBD_CFG_REC: {
		...
		lcfg_new = kzalloc(lustre_cfg_len(bufs.lcfg_bufcount, bufs.lcfg_buflen),
				   GFP_NOFS);
		if (!lcfg_new) {
			rc = -ENOMEM;
			goto out;
