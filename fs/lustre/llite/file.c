			...
			void __user *arg)
{
	...
	lump = kvzalloc(lum_size, GFP_KERNEL);
	if (lump == NULL)
		return -ENOMEM;
				 ...
				 unsigned long arg)
{
	...
	case LL_LEASE_RESYNC_DONE:
		...
		data = kzalloc(data_size, GFP_NOFS);
		if (!data) {
			rc = -ENOMEM;
			goto out_lease_close;
		}
...
out:
	...
	case LL_IOC_LADVISE: {
		...
		k_ladvise_hdr = kzalloc(sizeof(*k_ladvise_hdr), GFP_NOFS);
		...
		k_ladvise_hdr = kzalloc(alloc_size, GFP_NOFS);
		if (k_ladvise_hdr == NULL)
			return -ENOMEM;
...
out_ladvise:
	...
	case LL_IOC_PCC_DETACH: {
		...
		detach = kzalloc(sizeof(*detach), GFP_NOFS);
		if (detach == NULL)
			return -ENOMEM;
