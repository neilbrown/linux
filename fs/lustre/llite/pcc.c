...
pcc_fname_list_add(char *id, struct list_head *fname_list)
{
	...
	fname = kzalloc(sizeof(*fname), GFP_NOFS);
	if (fname == NULL)
		return -ENOMEM;

	fname->pmf_name = kzalloc(strlen(id) + 1, GFP_NOFS);
...
pcc_fname_list_parse(char *str, struct list_head *fname_list)
{
	...
	for (; rc == 0 && *str ; str = skip_spaces(str)) {
...
pcc_expression_parse(char *str, struct list_head *cond_list)
{
	...
	expr = kzalloc(sizeof(*expr), GFP_NOFS);
	if (expr == NULL)
		return -ENOMEM;
...
pcc_conjunction_parse(char *str, struct list_head *cond_list)
{
	...
	conjunction = kzalloc(sizeof(*conjunction), GFP_NOFS);
	if (conjunction == NULL)
		return -ENOMEM;
...
static int pcc_id_parse(struct pcc_cmd *cmd, const char *id)
{
	...
	cmd->u.pccc_add.pccc_conds_str = kzalloc(strlen(id) + 1, GFP_NOFS);
...
pcc_dataset_rule_init(struct pcc_match_rule *rule, struct pcc_cmd *cmd)
{
	...
	rule->pmr_conds_str = kzalloc(strlen(cmd->u.pccc_add.pccc_conds_str) + 1,
				      GFP_NOFS);
...
pcc_cmd_parse(char *buffer, unsigned long count)
{
	...
	cmd = kzalloc(sizeof(*cmd), GFP_NOFS);
	if (cmd == NULL) {
		rc = -ENOMEM;
		goto out;
	}
				  ...
				  bool *cached)
{
	...
	pathname = kzalloc(PATH_MAX, GFP_NOFS);
	if (pathname == NULL)
		return -ENOMEM;
		      ...
		      unsigned int flags, bool *cached)
{
	...
	old_cred = override_creds(pcc_super_cred(inode->i_sb));
	rc = vfs_getattr(&ll_i2pcci(inode)->pcci_path, &stat, request_mask,
			    flags);
	revert_creds(old_cred);
	if (rc)
		goto out;
