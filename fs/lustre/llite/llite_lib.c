...
void ll_finish_md_op_data(struct md_op_data *op_data)
{
	...
	fscrypt_free_ctx(op_data->op_file_encctx, op_data->op_file_encctx_size);
	kfree(op_data);
}
			...
			struct lov_user_md **kbuf)
{
	...
	*kbuf = kvzalloc(lum_size, GFP_KERNEL);
	if (*kbuf == NULL)
		return -ENOMEM;
