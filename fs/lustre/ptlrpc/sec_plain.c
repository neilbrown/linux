		       ...
		       int msgsize)
{
	...
	if (!req->rq_reqbuf) {
		...
		req->rq_reqbuf = kvzalloc(alloc_len, GFP_KERNEL);
		if (!req->rq_reqbuf)
			return -ENOMEM;
		       ...
		       int msgsize)
{
	...
	req->rq_repbuf = kvzalloc(alloc_len, GFP_KERNEL);
	if (!req->rq_repbuf)
		return -ENOMEM;
			 ...
			 int segment, int newsize)
{
	...
	if (req->rq_reqbuf_len < newbuf_size) {
		...
		newbuf = kvzalloc(newbuf_size, GFP_KERNEL);
		if (newbuf == NULL)
			return -ENOMEM;
...
int plain_alloc_rs(struct ptlrpc_request *req, int msgsize)
{
	...
	if (rs) {
		...
		rs = kvzalloc(rs_size, GFP_KERNEL);
		if (rs == NULL)
			return -ENOMEM;
