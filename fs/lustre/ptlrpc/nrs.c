				 ...
				 enum ptlrpc_nrs_ctl opc, void *arg)
{
	...
	return policy->pol_desc->pd_ops->op_policy_ctl != NULL ? policy->pol_desc->pd_ops->op_policy_ctl(policy,
													 opc,
													 arg) : -ENOSYS;
}
			       ...
			       struct ptlrpc_nrs_pol_desc *desc)
{
	...
	policy = kzalloc_node(sizeof(*policy), GFP_NOFS,
			      cfs_cpt_spread_node(svcpt->scp_service->srv_cptable, svcpt->scp_cpt));
	if (policy == NULL)
		return -ENOMEM;
...
static int nrs_svcpt_setup_locked(struct ptlrpc_service_part *svcpt)
{
	...
	svcpt->scp_nrs_hp = kzalloc_node(sizeof(*svcpt->scp_nrs_hp), GFP_NOFS,
					 cfs_cpt_spread_node(svcpt->scp_service->srv_cptable, svcpt->scp_cpt));
