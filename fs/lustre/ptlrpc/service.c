...
ptlrpc_alloc_rqbd(struct ptlrpc_service_part *svcpt)
{
	...
	rqbd = kzalloc_node(sizeof(*rqbd), GFP_NOFS,
			    cfs_cpt_spread_node(svc->srv_cptable, svcpt->scp_cpt));
	if (rqbd == NULL)
		return NULL;
	...
	rqbd->rqbd_buffer = kvmalloc_node(svc->srv_buf_size,
...
static void ptlrpc_at_timer(struct timer_list * data)
{
	...
	svcpt = from_timer(svcpt, data, scp_at_timer);
				    ...
				    struct ptlrpc_service_part *svcpt, int cpt)
{
	...
	array->paa_reqs_array = kzalloc_node(sizeof(struct list_head) * size,
					     GFP_NOFS,
	...
	array->paa_reqs_count = kzalloc_node(sizeof(__u32) * size, GFP_NOFS,
					     cfs_cpt_spread_node(svc->srv_cptable, cpt));
	if (array->paa_reqs_count == NULL)
		goto failed;

	timer_setup(&svcpt->scp_at_timer, ptlrpc_at_timer,
			0);
...
static int ptlrpc_at_send_early_reply(struct ptlrpc_request *req)
{
	...
	if (reqcopy == NULL)
		return -ENOMEM;
	reqmsg = kvzalloc(req->rq_reqlen, GFP_KERNEL);
	if (!reqmsg) {
		rc = -ENOMEM;
		goto out_free;
	}
...
int ptlrpc_hr_init(void)
{
	...
	ratelimit_state_init(&watchdog_limit,
			     libcfs_watchdog_ratelimit * HZ, 3);
	...
	cfs_percpt_for_each(hrp, cpt, ptlrpc_hr.hr_partitions) {
		...
		hrp->hrp_thrs = kzalloc_node(hrp->hrp_nthrs * sizeof(*hrt),
					     GFP_NOFS,
					     cfs_cpt_spread_node(ptlrpc_hr.hr_cpt_table, cpt));
		if (hrp->hrp_thrs == NULL) {
			rc = -ENOMEM;
			goto out;
		}
