					  ...
					  enum ldlm_ns_type ns_type)
{
	...
	ns->ns_rs_buckets = kvcalloc(1 << ns->ns_bucket_bits,
				     sizeof(*ns->ns_rs_buckets), GFP_KERNEL);
	if (!ns->ns_rs_buckets)
		goto out_hash;
...
static bool ldlm_resource_extent_new(struct ldlm_resource *res)
{
	...
	res->lr_itree = kmem_cache_zalloc(ldlm_interval_tree_slab, GFP_NOFS);
...
static bool ldlm_resource_inodebits_new(struct ldlm_resource *res)
{
	...
	res->lr_ibits_queues = kzalloc(sizeof(*res->lr_ibits_queues),
				       GFP_NOFS);
