...
/*
 * Note that a dentry got changed.  We need to set d_version to the data version
 * number derived from the result of the operation.  It doesn't matter if
 * d_version goes backwards as we'll just revalidate.
 */
					     ...
					     struct dentry *dentry)
{
	if (!op->cumul_error.error)
		dentry->d_version =
			dir_vp->scb.status.data_version;
}
