		    ...
		    struct lnet_peer_net *peer_net)
{
	...
	while ((lpni = lnet_get_next_peer_ni_locked(peer, peer_net, lpni))) {
		 ...
		 */
		if (best_ni) {
			ni_is_pref = lnet_peer_is_pref_nid_locked(
				lpni, best_ni->ni_nid);
	...
	cfs_cpt_for_each(i, lnet_cpt_table()) {
		...
		list_for_each_entry_safe(rspt, tmp,
			...
			if (ktime_compare(now,
				...
				lnet_net_lock(i);
				the_lnet.ln_counters[i]->lct_health.
					lch_response_timeout_count++;
				lnet_net_unlock(i);
...
lnet_resend_pending_msgs_locked(struct list_head *resendq, int cpt)
{
	...
	while (!list_empty(resendq)) {
		...
		if (!lpni) {
			...
			lnet_net_lock(cpt);
			if (!rc)
				the_lnet.ln_counters[cpt]->lct_health.
					lch_resend_count++;
		}
	}
}
...
lnet_send_ping(lnet_nid_t dest_nid,
	       ...
	       void *user_data, lnet_eq_handler_t eqh, bool recovery)
{
	...
	if (lnet_drop_asym_route && for_me &&
		...
		if (rnet) {
			...
			list_for_each_entry(route, &rnet->lrn_routes, lr_list) {
				 ...
				 */
				while ((lpni = lnet_get_next_peer_ni_locked(
					gw, NULL, lpni)) != NULL) {
