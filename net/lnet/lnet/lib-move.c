			     ...
			     struct lnet_peer **gw_peer)
{
	...
	if (!route_found) {
		...
		/*
		 * find the best route.  Restrict the selection on the net of
		 * the local NI if we've already picked the local NI to send
		 * from.  Otherwise, let's pick any route we can find and then
		 * find a local NI we can reach the route's gateway on.  Any
		 * route we select will be reachable by virtue of the
		 * restriction we have when adding a route.
		 */
