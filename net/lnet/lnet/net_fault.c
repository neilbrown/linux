		  ...
		  struct list_head *msg_list)
{
	...
	if (!all && rule->dl_msg_send * HZ > now)
		return;
...
static void
delay_timer_cb(struct timer_list * data)
{
	struct lnet_delay_rule *rule = from_timer(rule, data, dl_timer);
...
lnet_delay_rule_add(struct lnet_fault_attr *attr)
{
	...
	timer_setup(&rule->dl_timer, delay_timer_cb,
			0);
