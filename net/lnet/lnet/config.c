...
lnet_net_append_cpts(__u32 *cpts, __u32 ncpts, struct lnet_net *net)
{
	...
	if (net->net_cpts == NULL) {
		net->net_cpts = kcalloc(ncpts, sizeof(*net->net_cpts),
					GFP_NOFS);
		if (net->net_cpts == NULL)
			return -ENOMEM;
	...
	added_cpts = kcalloc(LNET_CPT_NUMBER, sizeof(*added_cpts), GFP_NOFS);
	if (added_cpts == NULL)
		return -ENOMEM;
	...
	if (j > 0) {
		...
		array = kcalloc(total_entries, sizeof(*array), GFP_NOFS);
		if (array == NULL) {
			rc = -ENOMEM;
			goto failed;
		}
...
lnet_ni_add_interface(struct lnet_ni *ni, char *iface)
{
	...
	ni->ni_interfaces[niface] = kzalloc(strlen(iface) + 1, GFP_NOFS);
			  ...
			  char *iface)
{
	...
	if (ncpts == 0) {
		...
		ni->ni_cpts = kcalloc(ncpts, sizeof(*ni->ni_cpts), GFP_NOFS);
		if (ni->ni_cpts == NULL)
			goto failed;
		    ...
		    bool use_tcp_bonding)
{
	...
	tokens = kzalloc(tokensize, GFP_NOFS);
	if (tokens == NULL) {
...
lnet_parse_ip2nets(const char **networksp, const char *ip2nets)
{
	...
	ipaddrs = kcalloc(nip, sizeof(*ipaddrs), GFP_NOFS);
	if (!ipaddrs) {
		rc = -ENOMEM;
