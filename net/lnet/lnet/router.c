...
lnet_router_discovery_complete(struct lnet_peer *lp)
{
	...
	if (!lp->lp_dc_error) {
		/*
		 * mark single-hop routes.  If the remote net is not
		 * configured on the gateway we assume this is intentional
		 * and we mark the gateway as multi-hop
		 */
		list_for_each_entry(route, &lp->lp_routes, lr_gwlist) {
			lnet_set_route_aliveness(route, true);
	    ...
	    time64_t when)
{
	...
	if (alive) {
		if (reset) {
			...
			u32 sensitivity = lpni->lpni_peer_net->
