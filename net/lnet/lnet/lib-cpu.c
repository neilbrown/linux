...
struct cfs_cpt_table *cfs_cpt_table_alloc(int ncpt)
{
	...
	cptab->ctb_nodemask = kzalloc(sizeof(*cptab->ctb_nodemask), GFP_NOFS);
	if (!cptab->ctb_nodemask)
		goto failed_alloc_nodemask;

	cptab->ctb_cpu2cpt = kcalloc(nr_cpu_ids, sizeof(*cptab->ctb_cpu2cpt),
				     GFP_NOFS);
	...
	cptab->ctb_node2cpt = kcalloc(nr_node_ids,
				      sizeof(*cptab->ctb_node2cpt), GFP_NOFS);
	...
	cptab->ctb_parts = kcalloc(ncpt, sizeof(*cptab->ctb_parts), GFP_NOFS);
	if (!cptab->ctb_parts)
	...
	for (i = 0; i < ncpt; i++) {
		...
		part->cpt_distance = kcalloc(cptab->ctb_nparts,
					     sizeof(*part->cpt_distance),
					     GFP_NOFS);
...
failed_setting_ctb_parts:
	...
	if (cptab->ctb_parts)
		kfree(cptab->ctb_parts);

failed_alloc_ctb_parts:
	if (cptab->ctb_node2cpt)
		kfree(cptab->ctb_node2cpt);

failed_alloc_node2cpt:
	if (cptab->ctb_cpu2cpt)
		kfree(cptab->ctb_cpu2cpt);
