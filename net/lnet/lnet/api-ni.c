...
LNetNIInit(lnet_pid_t requested_pid)
{
	...
	if (IS_ERR(the_lnet.ln_mt_eq)) {
		CERROR("Can't allocate monitor thread EQ: %ld\n",
		       PTR_ERR(the_lnet.ln_mt_eq));
		    the_lnet.ln_mt_eq = NULL;
		goto err_stop_ping;
	}
...
static void
lnet_ping_event_handle(struct lnet_event *event)
{
		     ...
		     struct lnet_process_id __user *ids, int n_ids)
{
	...
	eq = LNetEQAlloc(0, lnet_ping_event_handle);
	if (IS_ERR(eq)) {
		rc = PTR_ERR(eq);
