...
lnet_create_remote_nets_table(void)
{
	...
	hash = kcalloc(LNET_REMOTE_NETS_HASH_SIZE, sizeof(*hash), GFP_NOFS);
	if (hash == NULL) {
...
lnet_destroy_remote_nets_table(void)
{
	...
	kfree(the_lnet.ln_remote_nets_hash);
...
lnet_res_container_setup(struct lnet_res_container *rec, int cpt, int type)
{
	...
	rec->rec_lh_hash = kzalloc_node(LNET_LH_HASH_SIZE * sizeof(rec->rec_lh_hash[0]),
					GFP_NOFS,
...
lnet_ping_buffer_alloc(int nnis, gfp_t gfp)
{
	...
	pbuf = kzalloc(LNET_PING_BUFFER_SIZE(nnis), gfp);
	if (pbuf) {
		pbuf->pb_nnis = nnis;
	      ...
	      struct lnet_process_id __user *ids, int n_ids)
{
	...
	buf = kcalloc(n_ids, sizeof(*buf), GFP_NOFS);
	if (!buf)
		return -ENOMEM;
