...
lnet_peer_tables_create(void)
{
	...
	cfs_percpt_for_each(ptable, i, the_lnet.ln_peer_tables) {
		hash = kzalloc_node(LNET_PEER_HASH_SIZE * sizeof(*hash),
				    GFP_NOFS,
				    cfs_cpt_spread_node(lnet_cpt_table(), i));
		if (hash == NULL) {
...
lnet_peer_ni_alloc(lnet_nid_t nid)
{
	...
	lpni = kzalloc_node(sizeof(*lpni), GFP_NOFS,
...
lnet_peer_net_alloc(__u32 net_id)
{
	...
	lpn = kzalloc_node(sizeof(*lpn), GFP_NOFS,
			   cfs_cpt_spread_node(lnet_cpt_table(), CFS_CPT_ANY));
	if (!lpn)
		return NULL;
...
lnet_peer_alloc(lnet_nid_t nid)
{
	...
	lp = kzalloc_node(sizeof(*lp), GFP_NOFS,
			  cfs_cpt_spread_node(lnet_cpt_table(), CFS_CPT_ANY));
	if (!lp)
		return NULL;
...
lnet_peer_add_pref_nid(struct lnet_peer_ni *lpni, lnet_nid_t nid)
{
	...
	if (lpni->lpni_pref_nnids != 0) {
		...
		nids = kzalloc_node(size, GFP_NOFS,
				    cfs_cpt_spread_node(lnet_cpt_table(), lpni->lpni_cpt));
		if (!nids) {
			rc = -ENOMEM;
			goto out;
		}
				...
				struct lnet_ping_buffer *pbuf)
{
	...
	curnis = kcalloc(nnis, sizeof(*curnis), GFP_NOFS);
	addnis = kcalloc(nnis, sizeof(*addnis), GFP_NOFS);
	delnis = kcalloc(nnis, sizeof(*delnis), GFP_NOFS);
...
int lnet_get_peer_info(struct lnet_ioctl_peer_cfg *cfg, void __user *bulk)
{
	...
	rc = -ENOMEM;
	lpni_info = kzalloc(sizeof(*lpni_info), GFP_NOFS);
	if (!lpni_info)
		goto out_lp_decref;
	lpni_stats = kzalloc(sizeof(*lpni_stats), GFP_NOFS);
	if (!lpni_stats)
		goto out_free_info;
	lpni_msg_stats = kzalloc(sizeof(*lpni_msg_stats), GFP_NOFS);
	if (!lpni_msg_stats)
		goto out_free_stats;
