...
lnet_attempt_msg_resend(struct lnet_msg *msg)
{
	...
	my_slot =
		lnet_check_finalize_recursion_locked(
			msg,
	...
	case LNET_MSG_STATUS_OK:
		...
		if (lpni && msg->msg_rx_committed) {
			...
			if (lnet_isrouter(lpni) || the_lnet.ln_routing) {
				lnet_set_lpni_healthv_locked(
					lpni, LNET_MAX_HEALTH_VALUE);
			} else {
			   ...
			   enum lnet_msg_hstatus *hstatus)
{
	...
	CDEBUG(D_NET,
	       "src %s, dst %s: %s simulate health error: %s\n",
	       libcfs_nid2str(msg->msg_hdr.src_nid),
	       libcfs_nid2str(msg->msg_hdr.dest_nid),
	       lnet_msgtyp2str(msg->msg_type),
	       lnet_health_error2str(*hstatus));
...
again:
	 ...
	 */
	my_slot = lnet_check_finalize_recursion_locked(
		msg,
