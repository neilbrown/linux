...
again:
	...
	if (ps->ps_increasing) {
		...
		trips++;
		CDEBUG(D_NET,
		       "Another thread is allocating new %s pool, waiting %d seconds for her to complete. trips = %d\n",
...
static int kiblnd_hdev_get_attr(struct kib_hca_dev *hdev)
{
	...
	if (hdev->ibh_ibdev->ops.alloc_fmr &&
		...
		CERROR("IB device does not support FMRs nor FastRegs, can't register memoryd\n");
		return -ENXIO;
	}
