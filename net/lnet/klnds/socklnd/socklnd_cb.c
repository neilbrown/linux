		  ...
		  struct kvec *scratch_iov)
{
	...
	if (ksocknal_data.ksnd_stall_tx != 0)
		schedule_timeout_uninterruptible(ksocknal_data.ksnd_stall_tx * HZ);
		 ...
		 struct kvec *scratch_iov)
{
	...
	if (ksocknal_data.ksnd_stall_rx != 0)
		schedule_timeout_uninterruptible(ksocknal_data.ksnd_stall_rx * HZ);
...
int ksocknal_scheduler(void *arg)
{
	...
	rx_scratch_pgs = kzalloc_node(sizeof(*rx_scratch_pgs) * LNET_MAX_IOV,
				      GFP_NOFS,
				      cfs_cpt_spread_node(lnet_cpt_table(), sched->kss_cpt));
	if (!rx_scratch_pgs) {
		CERROR("Unable to allocate scratch pages\n");
		return -ENOMEM;
	}

	scratch_iov = kzalloc_node(sizeof(*scratch_iov) * LNET_MAX_IOV,
				   GFP_NOFS,
				   cfs_cpt_spread_node(lnet_cpt_table(), sched->kss_cpt));
	if (!scratch_iov) {
...
ksocknal_connd_get_route_locked(signed long *timeout_p)
{
	...
	list_for_each_entry(route, &ksocknal_data.ksnd_connd_routes,
		...
		if (*timeout_p == MAX_SCHEDULE_TIMEOUT ||
		    *timeout_p > (route->ksnr_timeout - now) * HZ)
			*timeout_p = (route->ksnr_timeout - now) * HZ;
	}
